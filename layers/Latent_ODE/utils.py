...
def sample_standard_gaussian(mu, sigma):
	...
	d = torch.distributions.normal.Normal(torch.tensor([0.0], device=device), torch.tensor([1.0], device=device))
	r = d.sample(mu.size()).squeeze(-1)
	return r * sigma.to(device=device, dtype=torch.float32) + mu.to(device=device, dtype=torch.float32)
...
def split_and_subsample_batch(data_dict, configs, data_type = "train"):
	if data_type == "train":
		# Training set
		if configs.extrap:
			processed_dict = split_data_extrap(data_dict, dataset = configs.dataset)
		else:
	...
	if (configs.sample_tp is not None) or (configs.cut_tp is not None):
		processed_dict = subsample_observed_data(processed_dict, 
			n_tp_to_sample = configs.sample_tp, 
			n_points_to_cut = configs.cut_tp)
...
def check_mask(data, mask):
    ...
    '''
    # Check that "mask" only contains zeros and ones
    assert mask.size() == data.size(), "Mask and data must have the same dimensions"

    # Check that all masked out elements are zeros
    masked_elements = data[mask == 0]
    assert torch.all(masked_elements == 0), "Masked elements must be zeros"
    
    # Check that mask contains only zeros and ones
    assert torch.all((mask == 0) | (mask == 1)), "Mask should contain only zeros and ones"
